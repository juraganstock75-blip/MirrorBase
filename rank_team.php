...
$pg = paginate($totalTeams, $perPage, $page);
...
$stmt = $pdo->prepare("
    SELECT
      ...
      COUNT(a.id)                  AS total
    FROM teams t
    INNER JOIN archives a ON a.team_id = t.id
...
?>
<div class="panel">
  <div class="panel-head"><h1>Team Rank</h1></div>
  <div class="panel-body">
    <div class="table-container">

      <div class="count-statistic">
        <center><span>Total Team: <?= number_format($totalTeams) ?></span></center>
      </div>
      ...
      <table class="mirror-table table-responsive">
        ...
        <tbody>
          <?php if (!$rows): ?>
            <tr><td colspan="6" class="muted" style="text-align:center;padding:20px">Belum ada data tim.</td></tr>
          <?php endif; ?>
          <?php foreach ($rows as $i => $r): ?>
            <tr>
              <td><?= ($pg['offset'] + $i + 1) ?>.</td>
              <td><a href="/search.php?q=<?= urlencode($r['name']) ?>"><?= e($r['name']) ?></a></td>
